...
if __name__ == "__main__":
    ...
    for caseIdx, caseName in enumerate(caseNameList):
        ...
        for tIdx in timeArange:
            ...
            if tIdx == 10: print(np.max(cwv))
            cwvFilter = np.ma.masked_array(cwv, cwv==0)
            if np.ma.count_masked(cwvFilter) == cwvFilter.size: 
                cwvFilter = cwv
            meanCWVs[tIdx] = np.nanmean(cwvFilter)
            stdCWVs[tIdx] = np.nanstd(cwvFilter)
            maxCWVs[tIdx] = np.nanmax(cwvFilter)
            
        axs[caseIdx//2, caseIdx%2].plot(hourTimeArange, meanCWVs, color='black')
        axs[caseIdx//2, caseIdx%2].plot(hourTimeArange, maxCWVs, color='red')
        ...
        if caseIdx%2 == 0: axs[caseIdx//2, caseIdx%2].set_ylabel("[kg/$m^2$]", fontsize=15)
        ...
        #
    fig.suptitle("LWP [kg/$m^2$]", fontsize=20)
    plt.savefig(f"lwpEvolution.jpg", dpi=250)
    plt.close()
