...
if __name__ == "__main__":
    ...
    cwvArange = np.arange(0, 101, 1)
    #levels = np.array([1, 10, 50, 100, 250, 500, 750, 1000])
    levels = np.linspace(0, 0.100, 21)
    ...
    for caseIdx, caseName in enumerate(caseNameList):
        ...
        for tIdx in timeArange:
            ...
            cwv = np.array(nc.Dataset(f"{config.waterPath}{caseName}/waterPath-{tIdx:06d}.nc")["qvPath"][0])
            distCWV[tIdx, :], _ = np.histogram(cwv, cwvArange)
        distCWV /= np.size(distCWV)
        print(np.max(distCWV))
        ...
        axs[caseIdx//2, caseIdx%2].set_ylim(0, 100)
        axs[caseIdx//2, caseIdx%2].set_yticks(np.arange(0, 100, 20))
    ...
    plt.savefig(f"PrecipitableWater-Pcolor.jpg", dpi=250)
    plt.close()
