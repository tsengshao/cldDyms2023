...
if __name__ == "__main__":
    ...
    for caseIdx, caseName in enumerate(caseNameList):
        ...
        maxSSTs = np.zeros(shape=timeArange.shape)
        for tIdx in timeArange:
            sst = np.array(vvmLoader.loadSurface(tIdx)["sprec"][0])*3600
            if tIdx == 10: print(np.max(sst))
            sstFilter = np.ma.masked_array(sst, sst<1)
            if np.ma.count_masked(sstFilter) == sstFilter.size: 
                sstFilter = sst
            meanSSTs[tIdx] = np.nanmean(sstFilter)
            stdSSTs[tIdx] = np.nanstd(sstFilter)
            maxSSTs[tIdx] = np.nanmax(sstFilter)
            
        axs[caseIdx//2, caseIdx%2].plot(hourTimeArange, meanSSTs, color='black')
        axs[caseIdx//2, caseIdx%2].plot(hourTimeArange, maxSSTs, color='red')
        ...
        if caseIdx%2 == 0: axs[caseIdx//2, caseIdx%2].set_ylabel("Precipitation [mm/hr]", fontsize=15)
    ...
    plt.savefig(f"PrecipEvolution.jpg", dpi=250)
    plt.close()
