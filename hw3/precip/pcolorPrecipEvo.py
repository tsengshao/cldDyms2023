...
if __name__ == "__main__":
    ...
    for caseIdx, caseName in enumerate(caseNameList):
        ...
        #distSST /= np.size(distSST)# - np.ma.count_masked(distSST))
        #distSST = np.ma.masked_array(distSST, distSST==0)
        #print(np.nanmax(distSST))
        ...
        axs[caseIdx//2, caseIdx%2].set_ylim(0, 120)
    ...
    plt.savefig(f"PreciEvolution-Pcolor.jpg", dpi=250)
    plt.close()
