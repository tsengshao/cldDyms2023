...
if __name__ == "__main__":
    ...
    sstArange = np.arange(297.0, 307.5, 0.5)
    rbCmap, norm = from_levels_and_colors(sstArange[:-1], [rbCmap(i/len(sstArange)) for i in range(len(sstArange))], extend="both")
    ...
    for caseIdx, caseName in enumerate(caseNameList):
        ...
        for tIdx in timeArange:
            yMeanSST = np.mean(vvmLoader.loadSurface(tIdx)["tg"][0], axis=(0))
    ...
    fig.colorbar(sstPcolor, cax=cbar_ax, extend="both", ticks=sstArange[::2])
    fig.suptitle("SST", fontsize=20)
    plt.savefig(f"SSTevolution-XC.jpg", dpi=250)
    plt.close()
